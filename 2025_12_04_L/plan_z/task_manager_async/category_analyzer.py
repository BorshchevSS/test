        ...
        while True:
            try:
                ...
                if task_data is None:
                    logger.info('Получен сигнал о завершении.'
                                'Процесс Анализатор завершает работу')
                    break
                row_index, task_desc = task_data
                logger.info(
                    f'Задача "{task_desc}" (строка {row_index}) '
                    f'передача в Процесс для анализа')
                new_category = self.analyze_task(task_desc)
                logger.info(f'Процесс определил Категорию для задачи'
                            f'"{task_desc}": {new_category}')
