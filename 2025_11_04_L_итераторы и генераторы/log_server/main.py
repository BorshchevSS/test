...
class LogAnalyzer:
    ...
    def __init__(self, file_path):
        ...
        self.MAX_HEAVY_REQUSTS = 10 #макс кол-во тяжёлых запросов
    ...
    def analyze(self):
        ...
        for entry in log_parser(self.file_path):
            ...
            self.status_counts[entry.method] += 1

            # логика проверки для ТОП 10 тяжёлых запросов
            if entry.size > HEAVY_REQUEST:
                ...
                if len(self.heavy_requests) < self.MAX_HEAVY_REQUSTS:
    ...
    def generate_report(self, template_path, output_path ="report.html"):
        ...
        template = env.get_template(os.path.basename(template_path)) # загружаем шаблон
        ...
        print("Отчет сохранен|Saved report")
