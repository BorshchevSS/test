...
class LimitCalls:
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if (self.first_call_time is None and self.time_win is not None):
                self.first_call_time = cur_time

            if (self.time_win is not None and self.first_call_time is not None and cur_time - self.first_call_time > self.time_win):
                self.call_count = 0
                self.first_call_time = None
            ...
            self.call_count += 1 #Увеличиваем счетчик
